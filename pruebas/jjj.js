async function enviarScript(scriptText) {
  ...
  const waitFor = async (sel, root = document, timeout = 10000) => {
    const t0 = performance.now();
    let el;
    while (!(el = root.querySelector(sel))) {
      if (performance.now() - t0 > timeout) throw new Error('No se encontró: ' + sel);
      await sleep(100);
    }
    return el;
  };
  ...
  const main = await waitFor('#main');
  const textarea = await waitFor('div[contenteditable="true"]', main);
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const sendBtn = await waitFor('[data-testid="send"], [data-icon="send"], [aria-label="Enviar"], [aria-label="Send"]', main);
    sendBtn.click();
