...
def title_case_no_accents(s: str) -> str:
    """Convierte a Title Case sin acentos, reemplazando guiones bajos con espacios."""
    if not isinstance(s, str):
        return ""
    # Reemplazar guiones bajos con espacios antes de procesar
    s_clean = s.replace("_", " ")
    return " ".join(w.capitalize() for w in s_clean.split())

def normalize_career_name(s: str) -> str:
    """Normaliza nombres de carreras de forma consistente: Title Case sin acentos con guiones bajos."""
    if not isinstance(s, str) or not s.strip():
        return ""
    # Limpiar texto y convertir a title case sin acentos
    clean_name = clean_text(s)
    title_name = title_case_no_accents(clean_name)
    # Reemplazar espacios con guiones bajos para consistencia
    return title_name.replace(" ", "_")
...
def extract_countries_from_dataframe(df: pd.DataFrame, location_col: str = "location") -> pd.DataFrame:
    ...
    df["pais"] = df["country"].map(title_case_no_accents)
...
def get_country_stats(df: pd.DataFrame, exclude_remote: bool = True) -> pd.DataFrame:
    ...
    return ranking[["pais", "count", "country"]]
